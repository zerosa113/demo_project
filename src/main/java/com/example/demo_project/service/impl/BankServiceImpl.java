...
/*	@Override
	...
	public void withdraw(Bank bank, int withdrawAmount) {
		...
		if (bank.getAmount() < withdrawAmount) {
			...
			System.out.println("存款餘額: " + bank.getAmount());
		}
	}
	@Override
...
/*	@Override
	...
	public BankRes withdraw(String account, int withdrawAmount) {
		int intAmount = 500;	//初始餘額
//		Bank bank= new Bank();
		BankRes res = new BankRes();
		if(withdrawAmount<0) {
			res.setMessage("withdrawAmount is negative");
			return res;
		}
		if(intAmount<withdrawAmount) {
			res.setMessage("withdrawAmount is negative");
			return res;
		}
		res.setAccount(account);
		res.setAmount(intAmount-withdrawAmount);
